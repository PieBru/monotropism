...
This repository holds a growing catalogue of research drafts and practical guides — each with rendered **PDFs** alongside its Markdown source, available in English and several translations. The live, always-current list is at **https://piebru.github.io/monotropism/**.
...
**Status:** the full grid is published in **English**, **Italian** and **German** — every life stage (row) and every counterpart (column), 54 printable two-page cell sheets plus 16 cumulative summaries (one per row and one per column), each rendered as HTML and PDF. The Italian and German grids are AI-assisted translations; the English original is the source of truth, and further languages are planned. Gender (male/female) is handled as a cross-cutting callout inside each sheet rather than a separate axis; some cells are marked **N/A** (e.g. a pre-school child has no *colleagues*) and are intentionally left blank.
